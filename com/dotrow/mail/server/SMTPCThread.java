...
public class SMTPCThread extends Thread implements DatabaseInterface {
    ...
    public void run(){
        try{
                    ...
                    if( line.startsWith("RCPT TO") ){
                        ...
                        if( user.userExist() ){
                            ...
                            if( ld.isLocalDomain( MessageHandlerFactory.getDomain( msg.getFrom() ) ) ){
                                ...
                                log.debugClientThread(this, "Sending mail error ( user no exist ) for local user", 2);
                                ExistUser = false;
                                sendLine( ResponseCode.RC250 );
                                continue;
                            }else{
            ...
            log.debugClientThread(this ,e, 0);
            e.printStackTrace();
            end();
        }
    }
        ...
        for( w=0; w < msg.getTo().size() ; w++ ) {
            ...
            if( user.haveQuota( msg.getData() ) ){
                ...
                if( ld.isLocalDomain( MessageHandlerFactory.getDomain( msg.getFrom()) ) ){
                    ...
                    log.debugClientThread(this, "Sending mail error ( quota ) for local user", 2);
                    _return = true;
                }else{
                    _return = false;
                }
